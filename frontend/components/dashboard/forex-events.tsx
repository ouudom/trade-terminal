...
function ImpactBadge({ impact }: { impact: string }) {
  return (
    <Badge variant={impact === "High" ? "loss" : "partial"}>
      {impact}
    </Badge>
  )
}
...
export function ForexEventsPanel() {
  ...
  // Week filter (display) — default this week
  ...
  const loadStored = useCallback(async () => {
    ...
    setError(null)
    try {
      const res = await fetch(`${API}/forexfactory/events`)
  ...
  async function handleFetch(week: WeekValue) {
    ...
    try {
      const res = await fetch(
        `${API}/forexfactory/calendar?week=${encodeURIComponent(week)}`
      )
      if (!res.ok) throw new Error(`${res.status}`)
      // After fetching, switch the display filter to the fetched week
      setWeekFilter(week)
      await loadStored()
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : `Fetch failed for "${week}"`)
    } finally {
      setFetching(false)
    }
  }

  // Unique sorted currencies from loaded events
  ...
  // Compute Sunday-based date range for the selected weekFilter
  const weekDateRange = useMemo((): [string, string] => {
    const today = new Date()
    const daysSinceSunday = (today.getDay()) % 7  // Sunday=0
    const thisSunday = new Date(today)
    thisSunday.setDate(today.getDate() - daysSinceSunday)

    const offset =
      weekFilter === "last week" ? -7 :
      weekFilter === "next week" ?  7 : 0
  ...
  // Apply all filters including week range
  const filtered = useMemo(() => {
    const [from, to] = weekDateRange
    return events.filter((ev) => {
      // Week filter: only show events whose date falls in the selected week
      if (ev.date) {
  ...
  const grouped = useMemo(() => {
    const map: Record<string, ForexEvent[]> = {}
  ...
  return (
    <div className="flex flex-col gap-4 p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {/* Single fetch button with dropdown */}
        <div className="relative" ref={dropdownRef}>
          ...
          >
            ...
            {!fetching && <ChevronDown size={11} className={cn("transition-transform", dropdownOpen && "rotate-180")} />}
          </button>
        ...
        {loading ? (
          ...
          <div className="flex h-32 items-center justify-center text-xs text-muted-foreground">
            {events.length === 0
              ...
              : `No ${weekLabel.toLowerCase()} events${impactFilter !== "All" || selectedCurrencies.size > 0 ? " matching filters" : ""} — try Fetch.`}
          </div>
        ) : (
          <Table>
            <TableHeader>
