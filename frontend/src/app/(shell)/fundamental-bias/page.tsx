import { FundamentalBiasDashboard } from "@/components/features/fundamental-bias/fundamental-bias-dashboard"
...
function toBullets(key_drivers: string | null, invalidation_notes: string | null): string[] {
  ...
  if (invalidation_notes)
    lines.push(...invalidation_notes.split("\n").map((s) => s.trim()).filter(Boolean))
  return lines
}
...
export default async function FundamentalBiasPage() {
  ...
  try {
    const rows: SnapshotRow[] = await biasApi.getSnapshots()
    entries = rows.map((r) => ({
      symbol: r.symbol,
      name: r.name,
      changePct: 0,
