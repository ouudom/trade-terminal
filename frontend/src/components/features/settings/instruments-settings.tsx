...
import { useInstruments } from "@/lib/hooks/use-instruments"
...
export function InstrumentsSettings() {
  const { instruments, loading, error, refetch, toggleActive } = useInstruments()
            ...
            <TableBody>
              {filtered.map((instrument) => (
                ...
                >
                  ...
                  <TableCell className="text-muted-foreground">{instrument.name}</TableCell>
                  <TableCell>
                    <span
                      className={cn(
                        "inline-flex items-center rounded px-1.5 py-0.5 font-mono text-[10px] uppercase tracking-wide",
                        assetClassStyle[instrument.asset_class] ??
